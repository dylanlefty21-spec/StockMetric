...
if not st.user.is_logged_in:
    ...
    html, body, [class*="css"] { font-family: 'Inter', sans-serif; background: #0a0d14; color: #e2e8f0; }
    ...
    with col2:
        ...
        st.markdown("""
        <div style='text-align:center; padding: 48px 40px; background:#141828;
                    border:1px solid #1e2640; border-radius:24px;'>
          <div style='font-size:48px; margin-bottom:12px;'>📊</div>
          <div style='font-size:32px; font-weight:900; background:linear-gradient(90deg,#6366f1,#38bdf8);
                      -webkit-background-clip:text; -webkit-text-fill-color:transparent; margin-bottom:8px;'>
            StockMetric
          </div>
          <div style='font-size:15px; color:#64748b; margin-bottom:32px; line-height:1.6;'>
            Smart stock analysis for beginner investors.<br>Sign in to save your watchlist, alerts, and portfolio.
          </div>
        </div>
        """, unsafe_allow_html=True)
...
# ── Watchlist & Alerts (session-based) ────────────────────────────────────────

# ── Persistent Storage ────────────────────────────────────────────────────────
from storage import (load_data, save_data, get_watchlist, save_watchlist,
                     get_alerts, save_alerts, get_portfolio, save_portfolio)

# Load from disk on first run
if "initialized" not in st.session_state:
    saved = load_data()
    st.session_state["watchlist"]        = saved.get("watchlist", ["AAPL", "MSFT", "NVDA"])
    st.session_state["alerts"]           = saved.get("alerts", {})
...
with st.sidebar:
    ...
    st.markdown("<div style='font-size:12px; color:#475569; margin-bottom:8px;'>Smart stock analysis for everyone</div>", unsafe_allow_html=True)

    # User info
    user_name = st.user.get("name", "Investor")
    user_email = st.user.get("email", "")
    st.markdown(f"""
    <div style='background:#1a1d2e; border:1px solid #1e2640; border-radius:10px;
                padding:10px 14px; margin-bottom:16px;'>
      <div style='font-size:12px; font-weight:700; color:#e2e8f0;'>👋 {user_name}</div>
    ...
    alert_count = len(st.session_state.get("triggered_alerts", []))
    alert_badge = f" 🔔 {alert_count}" if alert_count > 0 else ""

    PAGES = ["🏠  Home", "🔬  Deep Dive", "📊  Compare", "⭐  Watchlist",
             f"🔔  Alerts{alert_badge}", "💼  Portfolio", "📰  News", "📚  Learn"]

    nav_default = st.session_state.get("nav", "🏠  Home")
    nav_idx = 0
    for i, p in enumerate(PAGES):
        if p.startswith(nav_default[:4]):
            nav_idx = i
            break

    page = st.radio("", PAGES, label_visibility="collapsed", index=nav_idx)
    ...
    with c2:
        ...
        if st.button("Analyze →", use_container_width=True):
            if quick_ticker:
                ...
                st.session_state["nav"] = "🔬  Deep Dive"
                st.rerun()
            else:
                st.warning("Please enter a ticker symbol first, e.g. AAPL")
...
# ════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='font-weight:800; color:{TEXT};'>🔔 Alerts</h2>", unsafe_allow_html=True)
    st.markdown(f"<p style='color:{SUBTEXT}; margin-bottom:20px;'>Get notified when your stocks hit key conditions</p>", unsafe_allow_html=True)

    alerts = st.session_state["alerts"]

    st.markdown("<div class='sec-label'>CREATE A NEW ALERT</div>", unsafe_allow_html=True)
    a1, a2, a3, a4 = st.columns([1.5, 1.5, 1.5, 1])
    with a1:
        alert_ticker = st.text_input("Stock Ticker", placeholder="e.g. AAPL").upper().strip()
    with a2:
        alert_type = st.selectbox("Alert me when...", [
            "Turns BULLISH", "Turns BEARISH", "Goes NEUTRAL",
            "Score drops below 40", "Price hits target",
        ])
    with a3:
        price_target = None
        if alert_type == "Price hits target":
            price_target = st.number_input("Price Target ($)", min_value=0.01, value=100.0, step=0.01)
        else:
            st.markdown("<br>", unsafe_allow_html=True)
    with a4:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("+ Add Alert", use_container_width=True):
            if alert_ticker:
                key = f"{alert_ticker}_{alert_type}"
                alerts[key] = {"ticker": alert_ticker, "type": alert_type,
                                "price_target": price_target, "active": True}
    ...
    if alerts:
        st.markdown("<div class='sec-label'>CHECK ALL ALERTS NOW</div>", unsafe_allow_html=True)
        if st.button("🔄 Check All Alerts", use_container_width=False):
            from analyzer import analyze
            triggered = []
            prog = st.progress(0, text="Checking alerts...")
            unique_tickers = list(set(v["ticker"] for v in alerts.values() if v["active"]))
            cache = {}
            for i, t in enumerate(unique_tickers):
                prog.progress((i+1)/len(unique_tickers), text=f"Checking {t}...")
                try: cache[t] = analyze(t)
                except: pass
            for key, alert in alerts.items():
                if not alert["active"]: continue
                r = cache.get(alert["ticker"])
                if not r: continue
                fired, message = False, ""
                if alert["type"] == "Turns BULLISH" and r["verdict"] == "BULLISH":
                    fired, message = True, f"🟢 {alert['ticker']} is now BULLISH — Score: {r['overall']}/100"
                elif alert["type"] == "Turns BEARISH" and r["verdict"] == "BEARISH":
                    fired, message = True, f"🔴 {alert['ticker']} is now BEARISH — Score: {r['overall']}/100"
                elif alert["type"] == "Goes NEUTRAL" and r["verdict"] == "NEUTRAL":
                    fired, message = True, f"🟡 {alert['ticker']} is NEUTRAL — Score: {r['overall']}/100"
                elif alert["type"] == "Score drops below 40" and r["overall"] < 40:
                    fired, message = True, f"⚠️ {alert['ticker']} score dropped to {r['overall']}/100"
                elif alert["type"] == "Price hits target" and alert.get("price_target"):
                    if r["price"] >= alert["price_target"]:
                        fired, message = True, f"🎯 {alert['ticker']} hit ${alert['price_target']:.2f} — now ${r['price']:.2f}"
                if fired:
                    triggered.append({"key": key, "message": message, "ticker": alert["ticker"], "result": r})
            prog.empty()
            st.session_state["triggered_alerts"] = triggered

        triggered = st.session_state.get("triggered_alerts", [])
        if triggered:
            st.markdown("<div class='sec-label'>🚨 TRIGGERED ALERTS</div>", unsafe_allow_html=True)
            for t in triggered:
                r = t["result"]
                vc = r["verdict_color"]
                bc = "#052e16" if vc=="green" else ("#1c0a0a" if vc=="red" else "#1c1108")
                lc = GREEN if vc=="green" else (RED if vc=="red" else YELLOW)
                st.markdown(f"""<div style='background:{bc}; border:2px solid {lc}; border-radius:14px; padding:16px 20px; margin-bottom:10px;'>
                  <div style='font-size:15px; font-weight:700; color:{TEXT};'>{t["message"]}</div>
                  <div style='display:flex; gap:16px; margin-top:8px; flex-wrap:wrap;'>
                    <span style='font-size:12px; color:{SUBTEXT};'>Price: <b style='color:{TEXT};'>${r["price"]:.2f}</b></span>
                    <span style='font-size:12px; color:{SUBTEXT};'>Overall: <b style='color:{TEXT};'>{r["overall"]}/100</b></span>
                  </div>
                </div>""", unsafe_allow_html=True)
        elif "triggered_alerts" in st.session_state and len(st.session_state["triggered_alerts"]) == 0:
            st.success("✅ All clear — no alerts triggered right now.")

        st.markdown("<div class='sec-label'>YOUR ACTIVE ALERTS</div>", unsafe_allow_html=True)
        for key, alert in list(alerts.items()):
            c1, c2 = st.columns([9, 1])
            with c1:
                pt = f" at ${alert['price_target']:.2f}" if alert.get("price_target") else ""
                st.markdown(f"""<div style='background:{CARD}; border:1px solid {BORDER}; border-radius:12px; padding:14px 18px; margin-bottom:8px; display:flex; justify-content:space-between; align-items:center;'>
                  <div><span style='font-size:16px; font-weight:800; color:{TEXT};'>{alert["ticker"]}</span>
                  <span style='font-size:13px; color:{SUBTEXT}; margin-left:10px;'>{alert["type"]}{pt}</span></div>
                  <span style='font-size:11px; color:{GREEN}; font-weight:700;'>● ACTIVE</span>
                </div>""", unsafe_allow_html=True)
            with c2:
                if st.button("✕", key=f"del_{key}"):
                    del st.session_state["alerts"][key]
                    save_alerts(st.session_state["alerts"])
                    st.rerun()
    else:
        st.markdown(f"""<div style='background:{CARD}; border:1px solid {BORDER}; border-radius:14px; padding:40px; text-align:center; color:{SUBTEXT};'>
          <div style='font-size:32px; margin-bottom:12px;'>🔔</div>
          <div style='font-size:16px; font-weight:600; color:{TEXT}; margin-bottom:8px;'>No alerts yet</div>
          <div>Add your first alert above to start tracking your stocks</div>
        </div>""", unsafe_allow_html=True)
...
# ════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='font-weight:800; color:{TEXT};'>💼 Portfolio Tracker</h2>", unsafe_allow_html=True)
    st.markdown(f"<p style='color:{SUBTEXT}; margin-bottom:20px;'>Track your holdings and see your overall health score</p>", unsafe_allow_html=True)

    portfolio = st.session_state["portfolio"]

    # Add holding
    st.markdown("<div class='sec-label'>ADD A HOLDING</div>", unsafe_allow_html=True)
    pa, pb, pc, pd_ = st.columns([1.5, 1.5, 1.5, 1])
    with pa:
        port_ticker = st.text_input("Ticker", placeholder="e.g. AAPL", key="port_ticker").upper().strip()
    with pb:
        shares = st.number_input("Shares owned", min_value=0.01, value=1.0, step=0.01)
    with pc:
        avg_cost = st.number_input("Avg cost per share ($)", min_value=0.01, value=100.0, step=0.01)
    with pd_:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("+ Add", use_container_width=True, key="add_port"):
            if port_ticker:
                portfolio[port_ticker] = {"shares": shares, "avg_cost": avg_cost}
                st.session_state["portfolio"] = portfolio
                save_portfolio(portfolio)
                st.session_state.pop("port_results", None)
                st.success(f"Added {port_ticker}!")
            else:
    ...
    if not portfolio:
        st.markdown(f"""<div style='background:{CARD}; border:1px solid {BORDER}; border-radius:14px; padding:40px; text-align:center; color:{SUBTEXT};'>
          <div style='font-size:32px; margin-bottom:12px;'>💼</div>
          <div style='font-size:16px; font-weight:600; color:{TEXT}; margin-bottom:8px;'>No holdings yet</div>
        ...
        if "port_results" not in st.session_state:
            from analyzer import analyze
            port_results = {}
            prog = st.progress(0, text="Loading portfolio...")
            tickers = list(portfolio.keys())
            for i, t in enumerate(tickers):
                prog.progress((i+1)/len(tickers), text=f"Loading {t}...")
                try: port_results[t] = analyze(t)
                except: pass
            prog.empty()
        ...
        # Summary metrics
        total_invested = sum(h["shares"] * h["avg_cost"] for h in portfolio.values())
        total_value = sum(
            portfolio[t]["shares"] * port_results[t]["price"]
            for t in portfolio if t in port_results
        )
        total_pnl = total_value - total_invested
        total_pnl_pct = (total_pnl / total_invested * 100) if total_invested > 0 else 0
        avg_health = int(sum(port_results[t]["overall"] for t in port_results) / len(port_results)) if port_results else 0
        health_color = GREEN if avg_health >= 60 else (YELLOW if avg_health >= 40 else RED)
        pnl_color = GREEN if total_pnl >= 0 else RED

        st.markdown("<div class='sec-label'>PORTFOLIO SUMMARY</div>", unsafe_allow_html=True)
        s1, s2, s3, s4 = st.columns(4)
        with s1:
            st.markdown(f"""<div class='sm-card' style='text-align:center;'>
              <div style='font-size:11px; color:{SUBTEXT}; font-weight:700; letter-spacing:1px;'>TOTAL VALUE</div>
              <div style='font-size:32px; font-weight:800; font-family:JetBrains Mono,monospace; color:{TEXT};'>${total_value:,.0f}</div>
            </div>""", unsafe_allow_html=True)
        with s2:
            arrow = "▲" if total_pnl >= 0 else "▼"
            st.markdown(f"""<div class='sm-card' style='text-align:center;'>
              <div style='font-size:11px; color:{SUBTEXT}; font-weight:700; letter-spacing:1px;'>TOTAL GAIN/LOSS</div>
              <div style='font-size:32px; font-weight:800; font-family:JetBrains Mono,monospace; color:{pnl_color};'>{arrow}${abs(total_pnl):,.0f}</div>
              <div style='font-size:13px; color:{pnl_color};'>{arrow}{abs(total_pnl_pct):.1f}%</div>
            </div>""", unsafe_allow_html=True)
        with s3:
            st.markdown(f"""<div class='sm-card' style='text-align:center;'>
              <div style='font-size:11px; color:{SUBTEXT}; font-weight:700; letter-spacing:1px;'>AMOUNT INVESTED</div>
              <div style='font-size:32px; font-weight:800; font-family:JetBrains Mono,monospace; color:{TEXT};'>${total_invested:,.0f}</div>
            </div>""", unsafe_allow_html=True)
        with s4:
            st.markdown(f"""<div class='sm-card' style='text-align:center;'>
              <div style='font-size:11px; color:{SUBTEXT}; font-weight:700; letter-spacing:1px;'>PORTFOLIO HEALTH</div>
              <div style='font-size:32px; font-weight:800; font-family:JetBrains Mono,monospace; color:{health_color};'>{avg_health}/100</div>
            </div>""", unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<div class='sec-label'>YOUR HOLDINGS</div>", unsafe_allow_html=True)

        table = f"<div style='background:{CARD}; border:1px solid {BORDER}; border-radius:16px; overflow:hidden;'>"
        table += f"<table style='width:100%; border-collapse:collapse; font-size:14px;'>"
        table += f"""<tr style='background:{CARD2}; border-bottom:1px solid {BORDER};'>
            <th style='padding:14px 16px; text-align:left; font-size:11px; letter-spacing:1px; color:{SUBTEXT}; font-weight:700;'>STOCK</th>
            <th style='padding:14px 16px; text-align:center; font-size:11px; color:{SUBTEXT}; font-weight:700;'>SHARES</th>
            <th style='padding:14px 16px; text-align:center; font-size:11px; color:{SUBTEXT}; font-weight:700;'>AVG COST</th>
            <th style='padding:14px 16px; text-align:center; font-size:11px; color:{SUBTEXT}; font-weight:700;'>CURRENT</th>
            <th style='padding:14px 16px; text-align:center; font-size:11px; color:{SUBTEXT}; font-weight:700;'>VALUE</th>
            <th style='padding:14px 16px; text-align:center; font-size:11px; color:{SUBTEXT}; font-weight:700;'>GAIN/LOSS</th>
            <th style='padding:14px 16px; text-align:center; font-size:11px; color:{SUBTEXT}; font-weight:700;'>HEALTH</th>
            <th style='padding:14px 16px; text-align:center; font-size:11px; color:{SUBTEXT}; font-weight:700;'>VERDICT</th>
        </tr>"""

        for t, h in portfolio.items():
            r = port_results.get(t)
            if not r: continue
            cur_val = h["shares"] * r["price"]
            cost_val = h["shares"] * h["avg_cost"]
            pnl = cur_val - cost_val
            pnl_pct = (pnl / cost_val * 100) if cost_val > 0 else 0
            pc_ = GREEN if pnl >= 0 else RED
            pa_ = "▲" if pnl >= 0 else "▼"
            hc = GREEN if r["overall"] >= 60 else (YELLOW if r["overall"] >= 40 else RED)
            vc = r["verdict_color"]
            table += f"""<tr style='border-bottom:1px solid {BORDER};'>
                <td style='padding:14px 16px;'><div style='font-weight:800; font-size:15px; color:{TEXT};'>{t}</div><div style='font-size:11px; color:{SUBTEXT};'>{r["name"][:22]}</div></td>
                <td style='padding:14px 16px; text-align:center; color:{TEXT}; font-family:JetBrains Mono,monospace;'>{h["shares"]:.2f}</td>
                <td style='padding:14px 16px; text-align:center; color:{SUBTEXT}; font-family:JetBrains Mono,monospace;'>${h["avg_cost"]:.2f}</td>
                <td style='padding:14px 16px; text-align:center; color:{TEXT}; font-family:JetBrains Mono,monospace; font-weight:700;'>${r["price"]:.2f}</td>
                <td style='padding:14px 16px; text-align:center; color:{TEXT}; font-family:JetBrains Mono,monospace;'>${cur_val:,.0f}</td>
                <td style='padding:14px 16px; text-align:center; color:{pc_}; font-weight:700;'>{pa_}${abs(pnl):,.0f} ({pa_}{abs(pnl_pct):.1f}%)</td>
                <td style='padding:14px 16px; text-align:center; font-weight:800; font-size:18px; font-family:JetBrains Mono,monospace; color:{hc};'>{r["overall"]}</td>
                <td style='padding:14px 16px; text-align:center;'><span class='pill-{vc}'>{r["verdict"]}</span></td>
            </tr>"""
        table += "</table></div>"
        st.markdown(table, unsafe_allow_html=True)
        ...
        # Remove holding
        st.markdown("<div class='sec-label'>REMOVE A HOLDING</div>", unsafe_allow_html=True)
        remove_ticker = st.selectbox("Select stock to remove", list(portfolio.keys()), key="remove_sel")
...
# ════════════════════════════════════════════
    ...
    st.markdown(f"<h2 style='font-weight:800; color:{TEXT};'>📰 News</h2>", unsafe_allow_html=True)
    st.markdown(f"<p style='color:{SUBTEXT}; margin-bottom:20px;'>Latest headlines for any stock</p>", unsafe_allow_html=True)

    nc, nb = st.columns([3, 1])
    with nc:
        news_ticker = st.text_input("", placeholder="Enter ticker (e.g. AAPL)",
                                    label_visibility="collapsed",
                                    value=st.session_state.get("news_ticker", "AAPL")).upper().strip()
    with nb:
        news_btn = st.button("Get News →", use_container_width=True)

    if news_btn or "news_ticker" not in st.session_state or st.session_state.get("news_ticker") != news_ticker:
        ...
        with st.spinner(f"Fetching news for {news_ticker}..."):
            try:
                ...
                st.session_state["news_info"]  = info
            except Exception as e:
                st.error(f"Could not load news for {news_ticker}")
                st.stop()

    news  = st.session_state.get("news_data", [])
    info  = st.session_state.get("news_info", {})

    if info:
        name   = info.get("longName", news_ticker)
        price  = info.get("currentPrice") or info.get("regularMarketPrice", 0)
        chg    = info.get("regularMarketChangePercent", 0)
        chg_c  = GREEN if chg >= 0 else RED
        chg_a  = "▲" if chg >= 0 else "▼"
        st.markdown(f"""<div class='sm-card' style='margin-bottom:20px;'>
          <div style='display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;'>
            <div>
              <div style='font-size:22px; font-weight:800; color:{TEXT};'>{news_ticker}</div>
              <div style='font-size:14px; color:{SUBTEXT};'>{name}</div>
            </div>
            <div style='text-align:right;'>
              <div style='font-size:28px; font-weight:700; font-family:JetBrains Mono,monospace; color:{TEXT};'>${price:.2f}</div>
              <div style='color:{chg_c}; font-weight:600;'>{chg_a} {abs(chg):.2f}% today</div>
            </div>
          </div>
        </div>""", unsafe_allow_html=True)

    st.markdown("<div class='sec-label'>LATEST NEWS</div>", unsafe_allow_html=True)
    ...
    if not news:
        ...
        for article in news[:10]:
            try:
                ...
                link_html = f"<a href='{url}' target='_blank' style='color:{ACCENT}; font-size:12px; text-decoration:none;'>Read full article →</a>" if url else ""

                st.markdown(f"""<div style='background:{CARD}; border:1px solid {BORDER}; border-radius:14px; padding:18px 20px; margin-bottom:10px;'>
                  <div style='display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:6px;'>
                    <div style='font-size:15px; font-weight:700; color:{TEXT}; line-height:1.4; flex:1;'>{title}</div>
                  </div>
                  {f"<div style='font-size:13px; color:{SUBTEXT}; margin-bottom:8px; line-height:1.5;'>{summary[:200]}{'...' if len(summary)>200 else ''}</div>" if summary else ""}
                  <div style='display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;'>
                    <div style='font-size:11px; color:{SUBTEXT};'>{source}{" · " if source and pub_date else ""}{pub_date}</div>
                    {link_html}
                  </div>
                </div>""", unsafe_allow_html=True)
            except Exception:
